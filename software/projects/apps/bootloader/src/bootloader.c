...
void Bootloader_Task(void)
{
	...
	uint8_t rxByte;
	static bool request_cmd = true;

	switch(state){
		case ST_BEGIN:
		  /* wait for bootloader command (upload, verify, read, etc) */
		  status = BL_WAITING_FOR_CMD;
#ifdef PC_MODE
      if (true == request_cmd){
				/* print help */
				request_cmd = false;
        if(app == en_runapp_a){ printf("\r\nApp A upload slot selected\r\n"); }
        else { printf("\r\nApp B upload slot selected\r\n"); }
				printf("Send cmd: a = app A sel, b = app B sel, u = upload app binary,\r\n");
        printf("s = get status, r = reset\r\n");
			}
#endif
			if ((uartNum == 0) && UART0IsRxByteAvailable()){
				rxByte = UART0RxByte();
			} else if ((uartNum == 1) && UART1IsRxByteAvailable()){
				rxByte = UART1RxByte();
			} else {
				uartNum = ((uartNum + 1) % BL_NUM_UARTS);
				return;
			}
...
#ifdef PC_MODE
				if(app_status & en_memcheck_app_a_valid){
					printf("\r\nApp A valid");
				}else{
					printf("\r\nApp A invalid");
				}
				if(app_status & en_memcheck_app_b_valid){
					printf("\r\nApp B valid");
				}else{
					printf("\r\nApp B invalid");
				}
...
#else
				if(app_status & en_memcheck_app_a_valid){
					UART0TxByte((uint8_t)BL_IMG_A_VALID);
				}else{
					UART0TxByte((uint8_t)BL_ERR_IMG_A_INVALID);
				}
				if(app_status & en_memcheck_app_b_valid){
					UART0TxByte((uint8_t)BL_IMG_B_VALID);
				}else{
					UART0TxByte((uint8_t)BL_ERR_IMG_B_INVALID);
				}
#endif
			}
			if('u' == rxByte){
				...
				state = ST_XMODEM_DL;
				status = BL_WAITING_FOR_IMG;
				return;
			}
			if('x' == rxByte){
				/* Test only - corrupt selected upload slot */
#ifdef ENABLE_CRC_TEST
				Fram_GenerateSBEInAppSlot(app);
#endif

			}
			if('y' == rxByte){
				/* Test only - corrupt bootloader */
#ifdef ENABLE_CRC_TEST
				Fram_GenerateSBEInBootloader();
#endif
			}
			uartNum = ((uartNum + 1) % BL_NUM_UARTS);
			break;
		...
		case ST_XMODEM_DL:
#ifdef PC_MODE
			printf("\r\nUpload application .bin using XMODEM-CRC\r\n");
#endif
			if (app == en_runapp_a){
				bytesRcvd = XMODEM_ReceiveFile((uint8_t*)APP_A_START_ADDR, (uint8_t*)APP_A_CRC_LEN_ADDR, uartNum);
			} else if (app == en_runapp_b){
				bytesRcvd = XMODEM_ReceiveFile((uint8_t*)APP_B_START_ADDR, (uint8_t*)APP_B_CRC_LEN_ADDR, uartNum);
			}
			if (bytesRcvd > 0){
				/* Transfer complete */
#ifdef PC_MODE
				HAL_WaitMs(20);
			  	printf("nReceived %d bytes, copying to FRAM\r\n", bytesRcvd);
#endif
		    	state = ST_COPY_ROM;
				status = BL_SUCCESS;
			} else {
				state = ST_ERROR;
				status = BL_ERR_XMODEM;
			}
			break;
...
#ifdef PC_MODE
				printf("Write and verify complete. ");
#endif
				state = ST_RUN_APP;
				status = BL_SUCCESS;
			} else {
		...
		case ST_READ_MEM:
			/* read back user code space */
			//ReadMem(APPLICATION_START_ADDR, APPLICATION_END_ADDR);
			state = ST_BEGIN;
			status = BL_SUCCESS;
#ifdef PC_MODE
			/* print the help menu again next time around */
			request_cmd = true;
#endif
			break;
		...
		case ST_ERROR:
			...
			state = ST_BEGIN;
#else
			/* wait until status code is read */
			if (UART0IsRxByteAvailable())
			{
				if(UART0RxByte() == 's')
				{
					/* Send status (error code) */
					UART0TxByte((uint8_t)status);
					state = ST_BEGIN;
				}
	    	}
#endif
