...
/** Version */
#define HAL_CAN_VERSION (0x020001) // 2.01
...
// TA: Bugfix to BITMASK macro on Jan 20 2023 (VA416xx BSP 2.01)
#define BITMASK(val,msb,lsb) (val&(((ALLF<<(31-msb))>>(31-(msb-lsb)))<<lsb))
...
#define BITMASK_AND_SHIFTR(val,msb,lsb,resultLsbPos) (BITMASK(val,msb,lsb)>>(lsb-resultLsbPos))
#define BITMASK_AND_SHIFTL(val,msb,lsb,resultLsbPos) (BITMASK(val,msb,lsb)<<(resultLsbPos-lsb))
...
typedef enum {
  ...
  en_can_cmb_msgtype_STD11_REM_RESP  = 3,//set as rx buf *with* data and will autoresp back when matched
  ...
  en_can_cmb_msgtype_EXT29_REM_RESP  = 6 //set as rx buf *with* data and will autoresp back when matched
...
extern void     HAL_Can_ClearBuffer(can_cmb_t *can_cmb, const uint32_t count); //no bounds checking here
extern uint32_t HAL_Can_Setup(VOR_CAN_Type *myCAN, const can_config_t *myCANCfg, const IRQn_Type CAN_IRQn);
extern uint32_t HAL_Can_ConfigCMB_Rx(const hal_can_id29_or_11_t id, const en_can_cmb_msgtype_t msgType, can_cmb_t *can_cmb);
...
extern hal_can_mskBX_t HAL_Can_Make_maskBX(const hal_can_id29_t dontCareID, const bool dontCareRTR_SRR, const bool dontCareXRTR);
