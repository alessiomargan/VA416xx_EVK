...
/** Version */
#define HAL_DMA_VERSION (0x00020005) // 2.05
...
extern stc_dma_control_blk_t dma_blk;
...
// DMA master controls
extern hal_status_t HAL_DMA_Init(stc_dma_control_blk_t* ctrlBasePtr,
...
extern hal_status_t HAL_DMA_RegDoneCallback   (uint32_t ch, void (*func_ptr)(void*), void* handle);

// example usage
extern hal_status_t HAL_DMA_SetupPriAutoWordinc(uint32_t *source, uint32_t *dest, \
                                                uint32_t len, uint8_t ch, uint8_t dst_inc);
extern hal_status_t HAL_DMA_MemToMem(uint32_t *source, uint32_t *dest, uint32_t len, \
                                           uint8_t ch, uint8_t src_size, uint8_t dst_size);
extern hal_status_t HAL_DMA_SRAMtoPeriph8 (uint8_t*  source, uint8_t*  dest, uint32_t len, uint8_t ch);
extern hal_status_t HAL_DMA_SRAMtoPeriph16(uint16_t* source, uint16_t* dest, uint32_t len, uint8_t ch);
extern hal_status_t HAL_DMA_SRAMtoPeriph32(uint32_t* source, uint32_t* dest, uint32_t len, uint8_t ch);
extern hal_status_t HAL_DMA_PeriphToSRAM8 (uint8_t*  source, uint8_t*  dest, uint32_t len, uint8_t ch);
extern hal_status_t HAL_DMA_PeriphToSRAM16(uint16_t* source, uint16_t* dest, uint32_t len, uint8_t ch);
extern hal_status_t HAL_DMA_PeriphToSRAM32(uint32_t* source, uint32_t* dest, uint32_t len, uint8_t ch);

extern void         HAL_DMA_RestartPriBasic(uint8_t ch, uint32_t len);
