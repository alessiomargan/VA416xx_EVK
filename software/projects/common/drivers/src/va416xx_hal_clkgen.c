...
// *** does not generate LoClk interrupt on VA416x0_REVB, recommend do not use ***/
#if 0
 ...
 **
 ******************************************************************************/
hal_status_t HAL_Clkgen_Rearm(void)
{
...
/*******************************************************************************
 **
 ** @brief  Initializes the system clock source to external oscillator
 **         (no xtal feedback enabled)
 **
 ** @return hal_status_t Status of driver call (ok or initError)
 **
 ******************************************************************************/
hal_status_t HAL_Clkgen_XtalN(void)
{
  VOR_SYSCONFIG->PERIPHERAL_CLK_ENABLE |= (CLK_ENABLE_IRQ | CLK_ENABLE_CLKGEN);
  VOR_CLKGEN->CTRL0 &= ~(CLKGEN_CTRL0_CLKSEL_SYS_Msk | CLKGEN_CTRL0_SYS_CLK_LOST_DET_EN_Msk); // force to HBO clk
  __NOP();
  __NOP();
  VOR_CLKGEN->CTRL1 = CLKGEN_CTRL1_XTAL_N_EN_Msk;
  delay();
  (void)HAL_Clkgen_Rearm();
  VOR_CLKGEN->CTRL0 |= (2UL<<CLKGEN_CTRL0_REF_CLK_SEL_Pos) | (1UL<<CLKGEN_CTRL0_CLKSEL_SYS_Pos); // select XTAL minus to sysclk
  delay();
  if((VOR_CLKGEN->CTRL0 & CLKGEN_CTRL0_CLKSEL_SYS_Msk) == 0x0UL){
    // external clock fail
    SystemCoreClock = HBO;
    return hal_status_initError;
  }
  SystemCoreClock = EXTCLK;
  NVIC_EnableIRQ(LoCLK_IRQn);
...
/*******************************************************************************
 **
 ** @brief  Initializes the system clock source to external xtal
 **         (xtal feedback pin xtal_p enabled)
 **
 ** @return hal_status_t Status of driver call (ok or initError)
 **
 ******************************************************************************/
hal_status_t HAL_Clkgen_XtalOsc(void)
{
  VOR_SYSCONFIG->PERIPHERAL_CLK_ENABLE |= (CLK_ENABLE_IRQ | CLK_ENABLE_CLKGEN);
  VOR_CLKGEN->CTRL0 &= ~(CLKGEN_CTRL0_CLKSEL_SYS_Msk | CLKGEN_CTRL0_SYS_CLK_LOST_DET_EN_Msk); // force to HBO clk
  __NOP();
  ...
  (void)HAL_Clkgen_Rearm();
  VOR_CLKGEN->CTRL0 |= (1UL<<CLKGEN_CTRL0_REF_CLK_SEL_Pos) | (3UL<<CLKGEN_CTRL0_CLKSEL_SYS_Pos); // select XTAL amp to sysclk
  delay();
  if((VOR_CLKGEN->CTRL0 & CLKGEN_CTRL0_CLKSEL_SYS_Msk) == 0x0UL){
    // external clock fail
    SystemCoreClock = HBO;
...
/*******************************************************************************
 **
 ** @brief  Initializes the system clock source to PLL (xtal or xtal_n)
 **
 ** @param  u32Ctrl0Val - CLKGEN_CTRL0 register value (PLL settings)
 **
 ** @return hal_status_t Status of driver call
 **
 ** @note This function will set up the PLL but with a much reduced code size than
 ** HAL_Clkgen_Init() because it takes the PLL register value as in input vs. 
 ** running the algorithm to calculate the values based on input and output frequency. 
 ** To get the desired PLL settings, run HAL_Clkgen_Init() once, and grab the CTRL0
 ** register value from the debugger after calling. Then the reg value can be passed 
 ** into this function.
 **
 ******************************************************************************/
hal_status_t HAL_Clkgen_PLL(uint32_t u32Ctrl0Val)
{
  VOR_SYSCONFIG->PERIPHERAL_CLK_ENABLE |= (CLK_ENABLE_IRQ | CLK_ENABLE_CLKGEN);
  if((u32Ctrl0Val & CLKGEN_CTRL0_REF_CLK_SEL_Msk) == 0x1){
    VOR_CLKGEN->CTRL1 = CLKGEN_CTRL1_XTAL_EN_Msk; /* ref clk == xtal osc, so enable it in CTRL1 */
  } else {
    VOR_CLKGEN->CTRL1 = CLKGEN_CTRL1_XTAL_N_EN_Msk; /* ref clk == external clock, so enable it in CTRL1 */
  }
  u32Ctrl0Val |= CLKGEN_CTRL0_PLL_RESET_Msk;
  u32Ctrl0Val &= ~(CLKGEN_CTRL0_SYS_CLK_LOST_DET_EN_Msk | CLKGEN_CTRL0_CLKSEL_SYS_Msk);
  VOR_CLKGEN->CTRL0 = u32Ctrl0Val; /* PLL reg from running HAL_Clkgen_Init() */
  delay();
  VOR_CLKGEN->CTRL0 &= ~CLKGEN_CTRL0_PLL_RESET_Msk; /* PLL out of reset */
  delay();
  ...
  if(VOR_CLKGEN->STAT & (CLKGEN_STAT_FBSLIP_Msk | CLKGEN_STAT_RFSLIP_Msk)){
    ...
    if(VOR_CLKGEN->STAT & (CLKGEN_STAT_FBSLIP_Msk | CLKGEN_STAT_RFSLIP_Msk)){
      ...
      VOR_CLKGEN->CTRL0 = 0x00000030UL; /* default */
      VOR_CLKGEN->CTRL1 = 0x00000018UL; /* default */
      SystemCoreClock = HBO;
      return hal_status_initError;
    }
  }
  ...
  NVIC_EnableIRQ(LoCLK_IRQn);
  delay();
  VOR_CLKGEN->CTRL0 |= 2UL<<CLKGEN_CTRL0_CLKSEL_SYS_Pos; /* PLL -> sysclk */
  SystemCoreClockUpdate();

  // ADC clock (must be 2-12.5 MHz). Defaults to sysclk/8
  if(SystemCoreClock <= 50000000){
    VOR_CLKGEN->CTRL1 |= 0x1<<CLKGEN_CTRL1_ADC_CLK_DIV_SEL_Pos; // div 4
  }
  return hal_status_ok;
...
hal_status_t HAL_Clkgen_Init(hal_clkgen_init_t clkConfig)
{
  ...
  switch(clkConfig.pllcfg){
    ...
    case hal_pllcfg_enabled:
      ...
      if(clkConfig.pll_out_hz < PLLOUT_MIN_HZ){ 
        clkConfig.pll_out_hz = PLLOUT_MIN_HZ;
      }
      if(clkConfig.pll_out_hz > PLLOUT_MAX_HZ){ 
        clkConfig.pll_out_hz = PLLOUT_MAX_HZ;
      }
      if(clkConfig.xtalsel == hal_xtalsel_xtal_osc){
        pllInFreq = XTAL;
      } else {
        pllInFreq = EXTCLK;
      }
      if(calcPLL(pllInFreq, clkConfig.pll_out_hz) != 0){
        return hal_status_initError;
      }
  ...
  // setup SYSCLK lost detection
  if(clkConfig.lost_det_en){
    (void)HAL_Clkgen_Rearm();
  }
  ...
  switch(clkConfig.clksel){
    ...
    case hal_clksel_sys_xtal_n:
      c_assert(clkConfig.xtalsel == hal_xtalsel_xtal_n);
      if(clkConfig.xtalsel != hal_xtalsel_xtal_n){
        return hal_status_badParam;
      }
    ...
    case hal_clksel_sys_xtal_osc:
      c_assert(clkConfig.xtalsel == hal_xtalsel_xtal_osc);
      if(clkConfig.xtalsel != hal_xtalsel_xtal_osc){
        return hal_status_badParam;
      }
  ...
  if(clkConfig.lost_det_en){
    NVIC_EnableIRQ(LoCLK_IRQn); // enable SYSCLK lost interrupt
  }
  ...
  dbgprintf ("Begin PLL settings calculation\n");
	...
	for (nfi = (int)val; nfi < NF_MAX; ++nfi) {
		...
		if ((first) || (fabs (err) < merr * (1 + 1e-6)) || (fabs (err) < 1e-16)) {
			...
			if (! (((firstx) && (terr < 0)) || (fabs (err) < merr * (1 - 1e-6)) || ((MAX_VCO) && (no > x_no)))) continue;
    ...
    VOR_CLKGEN->CTRL0 &= ~(CLKGEN_CTRL0_PLL_CLKR_Msk | \
                          CLKGEN_CTRL0_PLL_CLKF_Msk | \
                          CLKGEN_CTRL0_PLL_CLKOD_Msk | \
                          CLKGEN_CTRL0_PLL_BWADJ_Msk);
