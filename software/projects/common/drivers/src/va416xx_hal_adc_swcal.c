...
// #ifndef ADC_CAL_DAC
// #error "must define ADC_CAL_DAC in hal_config.h as either VOR_DAC0 or VOR_DAC1"
// #endif
...
hal_status_t HAL_ADC_SoftwareCal(void)
{
  ...
  if(ADC_CAL_DAC == VOR_DAC0){ adcCtrl.chan_en = ADC_DAC0; }
  else{ adcCtrl.chan_en = ADC_DAC1; }
  ...
  adc_offset_corr = (int32_t)(adcOffsetCorr + (adcOffsetCorr ? 0.5f : -0.5f)); // rounding for integer conversion
