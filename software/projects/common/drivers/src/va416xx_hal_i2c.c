...
{
  uint16_t i, retry = 5;
  if(!(i2c->CTRL & I2C_CTRL_ENABLE_Msk)){ return; } // master not enabled
...
hal_status_t HAL_I2C_Reset(VOR_I2C_Type * const i2c)
{
  ...
  if(bank == I2C_INVALID_BANKNUM){ return hal_status_badParam; }
...
hal_status_t HAL_I2CM_Init(VOR_I2C_Type * const i2c, stc_i2c_masterInit_t const init)
{
  ...
  switch (init.speed)
  {
      ...
      if(I2C_CLK < 10000000) {return hal_status_initError;} // cannot do fast mode below 10mhz bus clk
  ...
  if(init.txfemd){ i2c->CTRL |= I2C_CTRL_TXFEMD_Msk; }
  if(init.rxffmd){ i2c->CTRL |= I2C_CTRL_RXFFMD_Msk; }
  if(I2CM_ALGFILT){ i2c->CTRL |= I2C_CTRL_ALGFILTER_Msk; }
  if(I2CM_DLGFILT){ i2c->CTRL |= I2C_CTRL_DLGFILTER_Msk; }
...
hal_status_t HAL_I2CS_Init(VOR_I2C_Type * const i2c, stc_i2c_slaveInit_t const init)
{
  ...
  if(init.tenBitMode){ i2c->S0_ADDRESS |= I2C_S0_ADDRESS_A10MODE_Msk; }
  if(init.addrMaskEnable){ i2c->S0_ADDRESSMASK = (init.addrMask & 0x7fe); }
  else { i2c->S0_ADDRESSMASK = 0x7fe; }
  
  i2c->S0_IRQ_ENB = 0;
  NVIC_EnableIRQ((IRQn_Type)(I2C0_SL_IRQn + 2*bank));
  
  if(init.txfemd){ i2c->S0_CTRL |= I2C_S0_CTRL_TXFEMD_Msk; }
  if(init.rxffmd){ i2c->S0_CTRL |= I2C_S0_CTRL_RXFFMD_Msk; }
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CM_WriteB(VOR_I2C_Type * const i2c, 
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CM_WriteBNoInt(VOR_I2C_Type * const i2c, 
                                  i2caddr_shifted_t i2cAddr,
                                  char *dataPtr, uint32_t len)
{
  ...
  if(HAL_I2C_Enclock(i2c) != hal_status_ok){ return hal_status_badParam; }
	if(!len){ return hal_status_badParam; }
  ...
  i2c->WORDS = len;
    ...
    i2c->DATA = *dataPtr++;
    len--;
    if((i2c->STATUS & I2C_STATUS_TXNFULL_Msk) == 0) { break; }
	}
	i2c->CMD = 0x3; // start with stop
  while(i2c->STATUS & I2C_STATUS_IDLE_Msk){}; // wait for xfer to start
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CM_ReadB(VOR_I2C_Type * const i2c,
                            i2caddr_shifted_t i2cAddr, 
                            char *dataPtr, uint32_t len)
{
  hal_status_t status;
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CM_ReadBNoInt(VOR_I2C_Type * const i2c,
                                 i2caddr_shifted_t i2cAddr, 
                                 char *dataPtr, uint32_t len)
{
  uint32_t tStart = (uint32_t)HAL_time_ms;
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CM_WrReadB(VOR_I2C_Type * const i2c,
                              i2caddr_shifted_t i2cAddr, 
                              char *wrDataPtr, uint32_t wrLen,
                              char *rdDataPtr, uint32_t rdLen)
{
  hal_status_t status;
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CM_WrReadBNoInt(VOR_I2C_Type * const i2c,
                                   i2caddr_shifted_t i2cAddr, 
                                   char *wrDataPtr, uint32_t wrLen,
                                   char *rdDataPtr, uint32_t rdLen)
{
  ...
  // check for possible errors
  if(HAL_I2C_Enclock(i2c) != hal_status_ok){ return hal_status_badParam; }
  if(!wrLen){ return hal_status_badParam; }
	if(!rdLen){ return hal_status_badParam; }
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CM_Write(VOR_I2C_Type * const i2c,
                            i2caddr_shifted_t i2cAddr, 
                            char *dataPtr, uint32_t len)
{
  uint8_t bank = GetBankNum(i2c);
  ...
  if(HAL_I2C_Enclock(i2c) != hal_status_ok){ return hal_status_badParam; }
	if(!len){ return hal_status_badParam; }
  if(dataPtr == NULL) return hal_status_badParam;
  if((i2c->CTRL & I2C_CTRL_ENABLED_Msk) == 0){ return hal_status_notInitialized; }
  if((i2c->STATUS & (I2C_STATUS_IDLE_Msk | I2C_STATUS_I2CIDLE_Msk)) !=
                    (I2C_STATUS_IDLE_Msk | I2C_STATUS_I2CIDLE_Msk)){
  ...
  if(len){ i2c->IRQ_ENB |= I2C_IRQ_ENB_TXREADY_Msk; }
  return hal_status_ok;
}
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CM_Read(VOR_I2C_Type * const i2c,
                           i2caddr_shifted_t i2cAddr, 
                           char *dataPtr, uint32_t len)
{
  uint8_t bank = GetBankNum(i2c);
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CM_WrRead(VOR_I2C_Type * const i2c,
                             i2caddr_shifted_t i2cAddr, 
                             char *wrDataPtr, uint32_t wrLen,
                             char *rdDataPtr, uint32_t rdLen)
{
  ...
  while(i2c->STATUS & I2C_STATUS_IDLE_Msk); // wait for xfer to start
  i2c->IRQ_CLR = 0xffffffff;
  i2c->IRQ_ENB = I2C_IRQ_ENB_RXREADY_Msk | I2C_IRQ_ENB_CLKLOTO_Msk |
                 I2C_IRQ_ENB_WAITING_Msk | I2C_IRQ_ENB_IDLE_Msk;
  if(wrLen){ i2c->IRQ_ENB |= I2C_IRQ_ENB_TXREADY_Msk; }
  return hal_status_ok;
}
 ...
 **
 ******************************************************************************/
#ifndef __HAL_DISABLE_I2C0_MASTER
void I2C0_MS_IRQHandler(void)
{
  ...
  if(VOR_I2C0->IRQ_END & I2C_IRQ_END_WAITING_Msk){
    ...
    if(aI2C[0].rdLen && (aI2C[0].rdBuf != NULL)){
      ...
      VOR_I2C0->IRQ_ENB &= ~(I2C_IRQ_ENB_TXREADY_Msk | I2C_IRQ_ENB_WAITING_Msk); // turn off TXREADY interrupt
...
void I2C0_MS_RX_IRQHandler(void){
  I2C0_MS_IRQHandler();
}
void I2C0_MS_TX_IRQHandler(void){
  I2C0_MS_IRQHandler();
}
#endif
 ...
 **
 ******************************************************************************/
#ifndef __HAL_DISABLE_I2C1_MASTER
void I2C1_MS_IRQHandler(void)
{
  ...
  if(VOR_I2C1->IRQ_END & I2C_IRQ_END_WAITING_Msk){
    ...
    if(aI2C[1].rdLen && (aI2C[1].rdBuf != NULL)){
      ...
      VOR_I2C1->IRQ_ENB &= ~(I2C_IRQ_ENB_TXREADY_Msk | I2C_IRQ_ENB_WAITING_Msk); // turn off TXREADY interrupt
...
void I2C1_MS_RX_IRQHandler(void){
  I2C1_MS_IRQHandler();
}
void I2C1_MS_TX_IRQHandler(void){
  I2C1_MS_IRQHandler();
}
#endif
 ...
 **
 ******************************************************************************/
#ifndef __HAL_DISABLE_I2C2_MASTER
void I2C2_MS_IRQHandler(void)
{
  ...
  if(VOR_I2C2->IRQ_END & I2C_IRQ_END_WAITING_Msk){
    ...
    if(aI2C[2].rdLen && (aI2C[2].rdBuf != NULL)){
      ...
      VOR_I2C2->IRQ_ENB &= ~(I2C_IRQ_ENB_TXREADY_Msk | I2C_IRQ_ENB_WAITING_Msk); // turn off TXREADY interrupt
...
void I2C2_MS_RX_IRQHandler(void){
  I2C2_MS_IRQHandler();
}
void I2C2_MS_TX_IRQHandler(void){
  I2C2_MS_IRQHandler();
}
#endif
 ...
 **
 ******************************************************************************/
hal_status_t HAL_I2CS_Listen(VOR_I2C_Type * const i2c,
                             char* dataPtr, uint16_t maxLen,
                             uint16_t threshLen)
{
  uint8_t bank = GetBankNum(i2c);
  ...
  // check for possible errors
  if(HAL_I2C_Enclock(i2c) != hal_status_ok){ return hal_status_badParam; }
  if(!maxLen){ return hal_status_badParam; }
	if(!threshLen){ return hal_status_badParam; }
  if(dataPtr == NULL){ return hal_status_badParam; }
  if((i2c->S0_CTRL & I2C_S0_CTRL_ENABLED_Msk) == 0){ return hal_status_notInitialized; }
  if((i2c->S0_STATUS & I2C_S0_STATUS_IDLE_Msk) == 0){ return hal_status_busy; }
 ...
 **
 ******************************************************************************/
extern hal_status_t HAL_I2CS_Respond(VOR_I2C_Type * const i2c,
                                     char* dataPtr, uint16_t len)
{
  uint8_t bank = GetBankNum(i2c);
 ...
 **
 ******************************************************************************/
#ifndef __HAL_DISABLE_I2C0_SLAVE
 ...
 **
 ******************************************************************************/
#ifndef __HAL_DISABLE_I2C1_SLAVE
 ...
 **
 ******************************************************************************/
#ifndef __HAL_DISABLE_I2C2_SLAVE
...
void I2C2_SL_RX_IRQHandler(void)
{
  I2C2_SL_IRQHandler();
}
void I2C2_SL_TX_IRQHandler(void)
{
  I2C2_SL_IRQHandler();
}
#endif
